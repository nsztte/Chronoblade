...
### 목표
- 10월 말까지 **포트폴리오 제출 가능한 완성 빌드 확보**
- 연출·디자인·사운드는 **간소화**하되, 전체 플레이 루프 완성에 집중
- “기능 완성 → 흐름 연결 → 최소 연출” 순서로 진행
...
### 주요 작업
- EnemySpawner 리젠 시스템 완성
  - 거리/시간 조건 기반 리스폰
  - OnEnemyDied 콜백 및 리스트 관리
- EnemyPatrolState 구현 및 FSM 연동
- BossFinalComboController 구현
  - 잡몹용 참고 → 보스 FSM 연동, Stagger/Slow 내성 처리
- 애니메이터 분리 (Humanoid ↔ 1인칭 검 팔 Generic)
  - Player_World(Humanoid): 이동/점프/하체 중심 애니메이션 유지
  - Player_FPArms_Sword(Generic): UpperBody의 검 브랜치(`SwordIdle`, `SwordSlash` 등) 이주
  - 공격 관련 파라미터 및 이벤트 수신 경로 FP 전용으로 재배선
  - `WeaponViewMode` 기반으로 World ↔ FP 전환 토글 구현
  - Humanoid ↔ FP 전환 시 블랙페이드 또는 크로스페이드 처리
- 전투 연출 1차 (VFX/SFX 최소 버전)
  - 기본 히트 효과, Swing/Impact SFX만 적용
  - HitStop / Shake 고정값 적용
- 콤보 판정(Perfect/Good/Miss) 피드백 연동

### 메모
- 전투 흐름 중심의 워밍업 주간
- 타격감·패턴 감각 복원에 집중  
- 완성된 보스 FSM 구조를 기준으로 연출 훅(Event) 확보  
- 애니메이터 분리는 하루 작업 분량 (프리팹/파라미터 이주 + 테스트 포함)
...
### 주요 작업
- 보스문 컷씬 제작 (기본 카메라 블렌드 2~3샷)
- Boss Phase 전환 컷씬 구성 (1~2초 단일 블렌드)
- Ending 씬 전환 컷씬 (블랙페이드 → 엔딩 카메라 → 타이틀 복귀)
- 컷씬 트리거 연결
  - 보스문 오픈 / Phase 진입 / 보스 사망 시점 호출
- UI 폴리싱
  - 색상, 정렬, 폰트 통일
  - 컷씬 중 HUD 숨김/표시 확인
...
### 메모
- 연출은 간단하게, “플로우” 완성에 집중
- 컷씬 구조는 5초 이내, 블렌드 위주로 구성
- UI 폴리싱은 톤 통일까지만 진행 (애니메이션 생략)
...
### 주요 작업
- Title → Loading → Chapter1 → Chapter2 → Boss → Ending 전체 루프 테스트
- 세이브 / 로드 / 컷씬 / 전투 간 충돌 테스트
- 퍼즐 클리어 후 로드 상태 유지 검증
- 보스전 세이브 차단 확인
- 영상 촬영용 빌드 제작
  - HUD 최소화, 60fps 고정
- README / 포트폴리오 문서 정리
...
- 플레이 가능한 최종 루프 완성
- 영상 촬영 및 문서화에 대비
- 11월 초에는 영상 편집 및 보정 중심으로 이동

---

## 종합 메모
- 전투/AI 중심 → 컷씬/연출 → 전체 루프 테스트 순으로 단계적 진행
- 연출 품질보다 “전체 완성도”와 “끊김 없는 흐름”을 우선
- 포폴용 핵심 강조점: FSM 구조, 세이브 시스템, 퍼즐/보스 연동, 카메라 설계, **애니메이터 분리 구조**
