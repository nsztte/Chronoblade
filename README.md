...
- UI 공통 프레임 & 버튼 이미지 제작 및 적용
  - 모든 UI에 공통으로 사용할 **프레임 + 반투명 배경 이미지** 제작  
  - 버튼 전용 공통 이미지 신규 제작 및 UI 전체 통일 적용  
  - **일시정지 UI**에 신규 공통 이미지 적용해 1차 테스트 완료  
  - HUD·옵션·인벤토리·상점·컨펌모달에서도 동일하게 사용할 기반 구축

- 옵션 UI 톤 보정 및 탭/텍스트 스타일 통일
  - 옵션 UI 탭 배경 이미지 교체 및 투명도 조정 
  - UI 프레임 톤에 맞춰 탭 버튼 스타일 일관성 확보  
  - 그룹 타이틀·라벨 폰트 변경(가독성 향상)  
  - 전체 텍스트 패딩/정렬/간격 재정비로 시각 통일성 강화

- 옵션 UI 토글 & 슬라이더 전면 개편
  - 기존 Unity Toggle 제거 → **UISwitcher / UINullableToggle 기반 토글 버튼**으로 교체  
  - AudioTabController에서 신규 토글 참조로 구조 변경  
  - `OnMuteChanged` 및 초기화 로직을 새 UI 방식에 맞게 수정  

- 슬라이더 스타일 재정비
  - Track Background 색상 → HUD 톤(D6F8EE 계열)에 맞게 변경  
  - Handle 색상·두께·명도 조정  
  - 트랙 두께 통일 및 전체적인 UI 톤 보정  

- 오디오 탭 전체 정리
  - 토글·슬라이더 레이아웃 일관성 확보  
  - 폰트 크기·스타일 조정  
  - 신규 UI 스타일과 옵션 패널 전체 분위기 통일

- 컨트롤 탭 UI 개선 (토글·슬라이더 적용)
  - invertY / toggleSprint / toggleCrouch  
    기존 체크박스 → **UISwitcher 토글**로 전면 교체  
  - ControlTabController 구조를 UISwitcher 기반으로 수정  
  - 마우스 감도 슬라이더도 새 스타일 적용  
  - 입력 매니저 반영 흐름(감도, Y축반전, 달리기/웅크리기 토글) 정상 확인

- 스크린 설정 드롭다운 전면 리디자인
  - 해상도·화면모드·주사율·V-Sync 드롭다운 UI 개선  
  - Unity 기본 Dropdown 제거 → 공통 프레임/버튼 기반으로 새로 제작  
  - 리스트 항목에도 동일한 패널/하이라이트 적용  
  - 텍스트 정렬·패딩·간격 최적화  
  - 옵션 UI 전체 톤과 100% 매칭되도록 색상·외곽선·투명도 조정

- 저장/불러오기 패널 UI 디자인 개선
  - 저장/불러오기 슬롯에 **공통 프레임 이미지 적용**  
  - 슬롯 전체를 카드형으로 재구성하여 시각적 가독성 대폭 향상  
  - 스크롤뷰 내부 패널과 슬롯 높이/패딩/썸네일 정렬 최적화  
  - “저장하기 / 불러오기” 두 탭 모두 동일한 레이아웃 품질로 통일  
  - 날짜·플레이타임·챕터명 표시 영역의 폰트와 색상도 새 UI 톤에 맞게 조정  
  - 전체 옵션 UI와 동일한 분위기로 완전 통합
...
- **MP 포션 아이템 모델 및 아이콘 추가**
  - 기존 HP 포션 모델 색상 변경으로 MP 포션 모델 제작  
  - WeaponIconCapture로 MP 포션 아이콘 캡처 후 ItemData에 적용  
  - MP_Potion 프리팹 신규 생성 및 데이터 연동  
  - 챕터1 씬에 MP_Potion_Pool 및 MP_Potion_Spawner 배치  

- **총기류 3종(피스톨/샷건/라이플) 로고 및 프리팹 제작**
  - PST/SGT/RFL 총기류 로고 아이콘 제작 및 ItemData에 적용  
  - 총기류 3종 전용 텍스처 제작 및 색상/라벨 분리  
  - 텍스처 기반 Material Variant 생성  
  - 총기류 3종 프리팹 구축 및 머테리얼 적용 완료  
  - 인벤토리·상점 UI에서 아이콘 정상 표시 확인  

- **아이템 이름 줄바꿈 시스템(‘|’) 도입 및 UI 표기 방식 개선**
  - ItemData.itemName에 ‘|’ 삽입하여 줄바꿈 표준화  
  - InventorySlot: ‘|’ → ‘\n’ 변환 후 표시  
  - ItemDetailPanel: 상세 UI에서도 ‘|’ → ‘\n’ 변환 적용  
  - ShopManager: 구매·판매·토스트 메시지에서 ‘|’ 제거된 이름 표시  
  - ItemPickup: 탄약 획득 메시지에서도 displayName으로 정리된 이름 적용  

- **UI 레이아웃 조정 (줄바꿈 대응)**
  - 슬롯 이름 텍스트 Pivot을 (0.5, 1)로 조정하여 두 줄 시 아래로 확장되도록 개선  
  - 슬롯 하단 오브젝트와의 간격 재정렬로 1줄/2줄 모두 자연스러운 레이아웃 유지  

- **키카드 아이콘 캡처 및 적용**
  - 블루/옐로우 키카드 아이콘 개별 렌더링 및 스프라이트 생성  
  - KeyCard ItemData에 전용 아이콘 적용  
  - 인벤토리·상점 UI에서 정상적으로 키카드 표시되도록 반영  

### 메모
- 라벨 기반 아이콘 방식 적용으로 탄약 박스 UI 가독성이 크게 향상됨  
- 줄바꿈 시스템 통합으로 아이템 이름 표기가 일관되고 안정적으로 유지됨  
...
- **보스 인트로 트리거 SaveableBehaviour 적용**
  - BossIntroTrigger에 SaveableBehaviour 상속 적용
  - 트리거 1회성 사용 여부를 active 상태 기반으로 저장/복원
  - 인트로 재실행 방지를 위한 트리거 비활성 상태 복원 처리
  - 보스 인트로 컷씬 트리거의 세이브/로드 안정성 확보

### 메모
- ResetToPreIntroState는 체크포인트 초기화용으로 구현 및 적용 완료
- 세이브 로드 시 보스 각성 상태(hasAwakened)에 따라 StartIdle / Idle 애니메이션 분기 처리
