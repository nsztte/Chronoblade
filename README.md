...
- 옵션용 SaveLoadPanel 및 SaveSlot 프리팹 구성 완료
  - SaveLoadPanel은 옵션 패널 하위에 배치되는 임베디드 구조로 설계
  - 저장/불러오기 버튼 그룹과 슬롯 리스트 영역을 분리하여 구조적 명확성 확보
  - SaveUI는 SaveOnly(저장 전용), LoadOnly(불러오기 전용) 모드 전환에 따라 슬롯 표현 방식 분기

- SaveSlot 레이아웃 디자인 설계 및 초기 구현
  - 사이버펑크2077 스타일을 참고한 저장 슬롯 UI 구조 설계
    - 좌측: 썸네일 (RawImage)
...
- SaveUI 정적 슬롯 방식으로 리팩토링
  - 기존의 동적 슬롯 생성 방식에서 벗어나 slotList 기반 5개 고정 슬롯으로 구조 재설계
  - RefreshSlots()에서는 slotIndex 기준으로 슬롯과 데이터를 매핑
    - 저장 모드(SaveOnly): 5~9번 수동 저장 슬롯만 표시
    - 불러오기 모드(LoadOnly): 퀵 저장 1개 + 최신 자동/수동 저장 4개 혼합 노출
  - 빈 슬롯은 frameRoot 비활성화 및 버튼 비인터랙티브 처리

- SaveSlot 개선
  - Init() 내부에서 SaveMeta에 따라 퀘스트/세이브타입/씬/플레이타임/저장시각 등의 정보 연동
  - 슬롯 클릭 시 저장/불러오기 동작 분기 및 ConfirmModal 연동 처리

- 세이브 슬롯 정렬 로직 개선
  - GetDisplaySlots: 퀵슬롯 존재 여부에 따라 자동/수동 슬롯 수 조정
    - 퀵슬롯 있으면 → 퀵 1 + 최신 저장 4개
    - 없으면 → 최신 저장 5개
  - 부족 시 5~9번 수동 슬롯 기준으로 null 패딩하여 UI 정렬 유지
  - GetManualSlotsOnly: (int, meta) 튜플 → SaveMeta 리스트로 리팩토링

- ToastUI 및 저장 파일 안정성 개선
  - 저장 후 `.json` → `.bak` 전환 로그 명확화
  - 파일 시스템 지연으로 인한 시각적 오해 방지
  - Toast 메시지 출력에 `WaitForSecondsRealtime` 적용으로 시간 정지 상태에서도 자연스럽게 페이드아웃 처리

### 메모
- 저장 슬롯 정렬 순서 문제는 slotList 순서대로 Init이 이뤄지기 때문이었으며,
  GetDisplaySlots에서 정렬된 데이터를 반환하되, slotList 순회와 index 매칭으로 해결
- 퀘스트 시스템이 없는 상태에서도 퀘스트 타이틀 표현은 메타 텍스트로 대체 예정
- 메인메뉴용 SaveUI는 별도로 구성 예정이며, 메인메뉴 구조 확정 후 재사용 여부 판단 필요
